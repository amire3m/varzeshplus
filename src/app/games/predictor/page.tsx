      ...
      <div className="max-w-[720px] mx-auto px-4 pt-6">
        ...
        {!authed && (
          <Link href="/login" className="block rounded-2xl border border-white/10 p-3 mb-4 text-center text-xs text-slate-300 hover:border-[#4AE183]/40" style={{ background: "rgba(74,225,131,0.08)" }}>
            برای ثبت پیش‌بینی و حضور در جدول <b style={{ color: "#4AE183" }}>وارد شوید ←</b>
          </Link>
        )}
        ...
        <div className="flex items-center gap-2 mb-4">
          {LEAGUES.map((l) => (
            <button key={l.slug} onClick={() => setLeague(l.slug)}
              ...
              style={league === l.slug ? { background: "linear-gradient(135deg, #4AE183, #FFD34D)", borderColor: "transparent" } : { background: "rgba(255,255,255,0.05)" }}>
              {l.label}
            </button>
          ))}
          {authed && <span className="mr-auto text-xs text-slate-400">امتیاز من: <b className="tabular" style={{ color: "#FFD34D" }}>{myTotal}</b></span>}
        </div>

        {msg && <p className="text-xs mb-3 text-center" style={{ color: msg.startsWith("✓") ? "#FFD34D" : "#E8385D" }}>{msg}</p>}
        ...
        {fxLoading ? (
          ...
          <div className="space-y-2.5 mb-6">
            {fixtures.map((fx) => {
              ...
              return (
                <div key={fx.id} className="rounded-2xl border border-white/10 p-4" style={{ background: "#101610" }}>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <button onClick={() => submit(fx)} className="px-4 py-1.5 rounded-full text-[11px] font-black text-white" style={{ background: "#4AE183" }}>
                  ...
                  {mineRow && !mineRow.pending && mineRow.actual && (
                    <div className="mt-2 pt-2 border-t border-white/5 text-[11px] text-slate-400">
                      ...
                      {" • "}
                      <b style={{ color: (mineRow.points ?? 0) > 0 ? "#FFD34D" : "#E8385D" }}>+{mineRow.points} امتیاز</b>
                    </div>
                  )}
