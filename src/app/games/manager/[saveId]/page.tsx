...
export default function ManagerDashboard() {
  ...
  if (!data) return <div className="min-h-screen flex items-center justify-center" style={{ background: "#0A0F0B", color: "#8FA1B5" }}>بارگذاری...</div>;
    ...
    <div className="min-h-screen pb-28" style={{ background: "#0A0F0B" }}>
      <div className="max-w-[1320px] mx-auto px-4 pt-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs px-2.5 py-1 rounded-full border border-white/10 text-white" style={{ background: "#4AE183" }}>{save.teamName}</span>
          <span className="text-xs text-slate-400">هفته {save.week} • فصل {save.season} • {save.points} امتیاز</span>
          <span className="mr-auto text-xs px-2 py-1 rounded-full" style={{ background: "rgba(190,229,3,0.15)", color: "#FFD34D" }}>{save.budget.toLocaleString("fa-IR")} بودجه</span>
        </div>

        {/* کارت بعدی */}
        {nextMatch && (
          <div className="rounded-2xl border border-white/10 p-4 mb-4 flex items-center justify-between" style={{ background: "linear-gradient(135deg, rgba(74,225,131,0.12), rgba(190,229,3,0.08))" }}>
            <div>
        ...
        <div className="flex gap-2 mb-4">
          {[
            ...
            <button key={t.k} onClick={() => setTab(t.k as any)} className={`px-4 py-1.5 rounded-full text-xs font-bold border ${tab === t.k ? "text-white" : "text-slate-400 border-white/10"}`} style={tab === t.k ? { background: "#4AE183", borderColor: "transparent" } : { background: "rgba(255,255,255,0.05)" }}>{t.l}</button>
          ))}
