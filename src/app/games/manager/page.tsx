    ...
    <div className="min-h-screen pb-28" style={{ background: "#0A0F0B" }}>
      <div className="max-w-[1320px] mx-auto px-4 pt-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ background: "linear-gradient(135deg, #4AE183, #FFD34D)" }}>FM</div>
          <div>
        ...
        {saves.length > 0 && (
          <div className="mb-6">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {saves.map((s) => (
                <Link key={s.id} href={`/games/manager/${s.id}`} className="rounded-2xl border border-white/10 p-4 hover:border-[#4AE183]/30 transition-colors" style={{ background: "#101610" }}>
                  ...
                  <div className="text-xs mt-1" style={{ color: "#FFD34D" }}>{s.budget.toLocaleString("fa-IR")} بودجه</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-white/10 p-5" style={{ background: "#101610" }}>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div>
              ...
              <select value={selectedLeague} onChange={(e) => { setSelectedLeague(e.target.value); setSelectedTeam(""); }} className="w-full mt-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white">
                {LEAGUES.map((l) => (
                  <option key={l.slug} value={l.slug} className="bg-[#0A0F0B]">{l.name}</option>
                ))}
              </select>
            </div>
            <div>
              ...
              <select value={selectedTeam} onChange={(e) => setSelectedTeam(e.target.value)} className="w-full mt-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white">
                <option value="" className="bg-[#0A0F0B]">— انتخاب کنید —</option>
                {leagueTeams.map((t) => (
                  <option key={t.slug} value={t.slug} className="bg-[#0A0F0B]">{t.name} — {t.city}</option>
                ))}
              </select>
            </div>
          </div>
