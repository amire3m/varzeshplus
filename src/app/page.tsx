...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: "#0A0F0B" }}>
      ...
      {/* ============ ۵. ویدیوهای برتر — ردیف افقی با عکس و تیتر جدا ============ */}
      <section className="w-full max-w-[1320px] mx-auto px-4 pt-5 pb-2">
        ...
        {(mixedNews ?? []).filter((n) => n.title.startsWith("ویدیو")).length >= 2 ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {(mixedNews ?? []).filter((n) => n.title.startsWith("ویدیو")).slice(0, 4).map((v, i) => (
              <a key={i} href={v.link} target="_blank" rel="noreferrer" className="rounded-2xl overflow-hidden group cursor-pointer border border-white/10 hover:border-[#4AE183]/40 transition-all" style={{ background: "#101610" }}>
                <div className="relative" style={{ aspectRatio: "16/9" }}>
                  {v.image ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={v.image} alt={v.title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.05]" loading="lazy" />
                  ) : (
                    <div className="absolute inset-0 flex items-center justify-center" style={{ background: "linear-gradient(135deg, #101610, #1D2718)" }}>
                      <span className="w-12 h-12 rounded-full flex items-center justify-center border border-white/20" style={{ background: "rgba(16,22,16,0.6)" }}>
                        <Play size={20} className="text-white" />
                      </span>
                    </div>
                  )}
                  {v.image && (
                    <span className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: "rgba(10,15,11,0.45)" }}>
                      <span className="w-12 h-12 rounded-full flex items-center justify-center border border-white/30" style={{ background: "rgba(16,22,16,0.7)" }}>
                        <Play size={20} className="text-white" />
                      </span>
                    </span>
                  )}
                  <span className="absolute bottom-2 left-2 text-[9px] font-black px-2 py-0.5 rounded tabular" style={{ background: "rgba(10,15,11,0.8)", color: "#FFD34D" }}>
                    ویدیو
                  </span>
                </div>
                <div className="p-3">
                  <p className="text-[12px] font-bold text-white leading-5 line-clamp-2 group-hover:text-[#4AE183] transition-colors">
                    {v.title.replace(/^ویدیو\s*\|?\s*/, "")}
                  </p>
                  {v.time && <span className="text-[10px] text-slate-500 mt-1 block">{v.time}</span>}
                </div>
              </a>
            ))}
          </div>
