  ...
  return (
    <PageShell badge="اخبار" activeDock="home">
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-3">
        {selected ? (
          <article className="rounded-[14px] border p-6" style={{ background: "#101610", borderColor: "rgba(255,255,255,0.1)" }}>
            ...
            <button onClick={() => { setSelected(null); window.history.replaceState({}, "", "/news"); }} className="mt-5 px-4 py-2 rounded-xl border text-sm font-bold transition-colors hover:bg-white/5" style={{ borderColor: "rgba(74,225,131,0.35)", color: "#4AE183" }}>بازگشت به فهرست</button>
          </article>
        ) : items.map((n) => (
          <Link key={n.id} href={`/news?id=${n.id}`} onClick={(e) => { e.preventDefault(); load(String(n.id)); window.history.replaceState({}, "", `/news?id=${n.id}`); }} className="block rounded-[14px] border border-white/10 p-4 transition-colors hover:bg-white/[0.04]" style={{ background: "#101610", borderColor: "rgba(255,255,255,0.1)" }}>
            <div className="flex items-center gap-2 mb-1.5">
              {n.isBreaking && <span className="text-xs font-bold px-2 py-0.5 rounded" style={{ background: "#E8385D", color: "#fff" }}>فوری</span>}
              {n.programTitle && <span className="text-xs px-2 py-0.5 rounded" style={{ background: "rgba(74,225,131,0.12)", color: "#4AE183" }}>{n.programTitle}</span>}
            </div>
