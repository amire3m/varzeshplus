...
export default function ProfilePage() {
  ...
  if (denied) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#0A0F0B" }}>
  ...
  if (!data) return <div className="min-h-screen flex items-center justify-center" style={{ background: "#0A0F0B", color: "var(--color-muted)" }}>...</div>;
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: "#0A0F0B" }}>
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        {/* کارت هویت */}
        <section className="rounded-2xl border border-white/10 overflow-hidden" style={{ background: "linear-gradient(160deg, rgba(74,225,131,0.15), #101610 60%)", borderColor: "rgba(74,225,131,0.25)" }}>
          <div className="p-5 flex items-center gap-4">
            <PlayerAvatar name={user.displayName || "کاربر"} size={72} color="#4AE183" round />
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-[10px] px-2 py-0.5 rounded-full font-black text-white" style={{ background: "linear-gradient(135deg,#4AE183,#FFD34D)" }}>سطح {user.level}</span>
                <div className="flex-1 h-1.5 rounded-full overflow-hidden max-w-[140px]" style={{ background: "rgba(255,255,255,0.08)" }}>
                  <div className="h-full rounded-full" style={{ width: `${xpInLevel / 10}%`, background: "linear-gradient(90deg, #4AE183, #FFD34D)" }} />
                </div>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-4 border-t border-white/5 text-center" style={{ background: "rgba(0,0,0,0.25)" }}>
            {[
              { label: "امتیاز", value: user.points, color: "#4AE183" },
              { label: "سکه", value: user.coins, color: "#FFD34D" },
        ...
        {saves.length > 0 && (
          <section className="rounded-2xl border border-white/10 p-4" style={{ background: "#101610" }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/games/manager" className="text-[11px] font-bold" style={{ color: "#4AE183" }}>مدیریت ←</Link>
            </div>
            <div className="space-y-1.5">
              {saves.slice(0, 3).map((s) => (
                <Link key={s.id} href={`/games/manager/${s.id}`} className="flex items-center gap-3 p-2.5 rounded-xl border border-white/5 hover:border-[#4AE183]/30 transition-colors" style={{ background: "rgba(255,255,255,0.03)" }}>
