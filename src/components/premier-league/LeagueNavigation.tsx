...
export function LeagueNavigation({ active, onChange }: { active: PLTab; onChange: (t: PLTab) => void }) {
  return (
    <nav className="sticky top-16 z-30 -mx-1 px-1 py-2 backdrop-blur-xl" style={{ background: "rgba(16,22,16,0.88)", borderBottom: "1px solid rgba(120,7,201,0.2)" }}>
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              style={isActive ? { background: "linear-gradient(135deg,#4AE183,#FFD34D)", boxShadow: "0 0 12px rgba(190,229,3,0.3)" } : undefined}
            >
              {t.label}
            </button>
          );
