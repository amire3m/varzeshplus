    ...
    <article className={`glass-panel overflow-hidden group flex flex-col ${big ? "" : "sm:flex-row"}`}>
      ...
      <div className="p-4 flex flex-col flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          <span className="text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider" style={{ background: "rgba(74,225,131,0.12)", color: "#4AE183" }}>{news.tag}</span>
          {news.teamId && (
        ...
        <h3 className={`font-bold leading-snug group-hover:underline decoration-[#4AE183] underline-offset-2 ${big ? "headline text-lg" : "text-sm"}`}>{news.title}</h3>
