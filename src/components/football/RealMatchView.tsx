  ...
  return (
    <div className="space-y-5">
      ...
      <section className="panel p-4 md:p-5">
        ...
        <div className="mt-2 text-center text-[10px]" style={{ color: "#4AE183" }}>دیتای واقعی Transfermarkt</div>
      </section>
      ...
      {tab === "lineup" && (
        <div className="space-y-4">
          <div className="flex items-center justify-center gap-4">
            <button onClick={() => setSide("home")} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all border ${side === "home" ? "text-white" : "text-white/55 border-white/10 hover:bg-white/5"}`} style={side === "home" ? { background: "linear-gradient(135deg, #4AE183, #FFD34D)", borderColor: "transparent" } : undefined}>{game.home.name}</button>
            <span className="tabular font-black text-lg" style={{ color: "var(--color-muted)" }}>VS</span>
            <button onClick={() => setSide("away")} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all border ${side === "away" ? "text-white" : "text-white/55 border-white/10 hover:bg-white/5"}`} style={side === "away" ? { background: "linear-gradient(135deg, #4AE183, #FFD34D)", borderColor: "transparent" } : undefined}>{game.away.name}</button>
          </div>
              ...
              {slot(starters, side === "away").map(({ p, x, y }) => (
                <div key={p.player_id} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center" style={{ left: `${x}%`, top: `${y}%`, width: 62 }}>
                  <span className="rounded-full p-[2px] mb-1" style={{ background: `linear-gradient(135deg, ${chosen.color}, rgba(255,255,255,0.35))` }}>
                    <span className="w-11 h-11 rounded-full flex items-center justify-center text-[11px] font-black" style={{ background: "#0D120D", color: chosen.color }}>{p.player_name?.slice(0, 2) ?? "??"}</span>
                  </span>
                    ...
                    {rows.map((a) => (
                      <tr key={a.player_id} className="border-t border-white/5 hover:bg-white/[0.04]">
                        <td className="px-2 py-2 font-bold">
                          <Link href={`/football/players/${a.player_id}`} className="hover:text-[#4AE183] transition-colors">{a.player_name ?? "—"}</Link>
                        </td>
                        <td className="px-2 py-2 text-center tabular">{a.minutes_played ?? 0}</td>
                        <td className="px-2 py-2 text-center tabular font-black" style={{ color: a.goals ? "#FFD34D" : "inherit" }}>{a.goals ?? 0}</td>
