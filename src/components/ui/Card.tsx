...
export function Card({ children, className = "", style }: { children: ReactNode; className?: string; style?: React.CSSProperties }) {
  return (
    <div className={`rounded-2xl border border-white/10 p-4 flex flex-col ${className}`} style={{ background: "rgba(16,22,16,0.9)", backdropFilter: "blur(8px)", ...style }}>
      {children}
    </div>
  );
}
...
export function SectionHeader({ title, href, linkLabel = "مشاهده همه" }: { title: string; href?: string; linkLabel?: string }) {
  ...
  return (
    <div className="flex items-center justify-between mb-3">
      ...
      <a href={href} className="text-[12px] font-bold hover:underline shrink-0" style={{ color: "#FFD34D" }}>{linkLabel}</a>
    </div>
  );
}
